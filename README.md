...
Multi-industry detailing site **and installable app** — Automotive, Marine, and Aviation from
one codebase. Next.js 14 (App Router), TypeScript, Tailwind, Framer Motion, SQLite.

The public marketing site lives at `/`. The Progressive Web App lives at `/app` (customers),
`/staff` (technicians), and `/admin` (owner). They share one codebase, one database, one
pricing engine and one design system — there is no second front end.
...
No environment variables are needed to run. Everything degrades gracefully: estimates and
bookings save to `./data.sqlite` immediately, and email, SMS, push, payments and the AI
assistant each stay switched off until configured (see `.env.example`).

Create the first owner account — there is no web route that can do this, deliberately:

```bash
npm run create-admin -- "Your Name" you@example.com
```

It prints a generated password once. Sign in at `/login` and change it.

```bash
npm run build       # production build
npm start           # serve the build
npm run typecheck   # tsc --noEmit
npm test            # estimate API smoke tests   (needs a running server)
npm run test:app    # auth / RBAC / booking tests (needs a running server)
...
**Installable** on iOS, Android, Windows and macOS via `public/manifest.webmanifest` and
`public/sw.js`. iOS has two platform quirks worth knowing: it never fires
`beforeinstallprompt` (the app shows Add-to-Home-Screen instructions instead), and push
notifications only work once the app has been added to the home screen (iOS 16.4+).

### Where the important logic lives

| Concern | File | Note |
|---|---|---|
| Schema | `lib/db.ts` | All SQL is here and in `lib/repo/*` — nowhere else |
| Auth | `lib/auth.ts` | scrypt; only the SHA-256 of a session token is stored |
| Authorisation | `lib/rbac.ts`, `lib/api.ts`, `lib/guards.ts` | Routes declare who may call them |
| No double booking | `lib/availability.ts` → `assertBookable()` | Re-checked at submit, not just at render |
| Pricing | `lib/pricing.ts` | Shared by client and server; **the server always recomputes** |
| Uploads | `lib/uploads.ts` | Magic-byte sniffing; client filenames are never used |
...
| File | What it covers |
|---|---|
| `PWA-AUDIT.md` | The app build: architecture, APIs, database, security, what remains |
| `VISUAL-SYSTEM.md` | The visual layer: textures, motion, readability rules, honesty rules |
...
composites a scrim, because the grey palette was measured against `#050505`) and
the **honesty rules** (why the before/after gallery is empty and why the mosaic
