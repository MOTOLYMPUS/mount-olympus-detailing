        ...
        >
          ...
          >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-6 sm:px-8 sm:pb-8">
              ...
              {/* Step transition.
                  `mode="wait"` so the outgoing step is fully gone before the
                  next mounts — overlapping them would double the focusable
                  controls inside the dialog for the length of the crossfade,
                  which the focus trap in lib/useDialog.ts would then include in
                  its Tab cycle. Opacity + translateY only; `key={step}` is what
                  makes AnimatePresence see a swap at all. */}
