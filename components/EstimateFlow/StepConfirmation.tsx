...
export default function StepConfirmation({
  ...
  // Carry the estimate's email (so registration links to it) and its first
  // service (so booking opens pre-selected) into the sign-up → book handoff.
  const firstService = form.serviceIds[0];
  const bookTarget = `/app/book${firstService ? `?service=${encodeURIComponent(firstService)}` : ''}`;
  const bookHref = `/register?email=${encodeURIComponent(form.email)}&next=${encodeURIComponent(
    bookTarget
  )}`;
  ...
  return (
    <div className="text-center">
      ...
      {/* ── Book a real slot ────────────────────────────────────────────────
          Hands the customer into the NATIVE booking flow (managed from /admin),
          not an external calendar. They create an account with the email this
          estimate was filed under, which the register route uses to seed their
          garage from this very estimate — so they land in booking with their
          vehicle and service already chosen, and pick from genuinely open times
          (business hours, buffers, travel and existing jobs all accounted for).
          `next` is a same-origin app path; the register form re-validates it. */}
      <div className="mt-7 flex flex-col gap-3 sm:flex-row-reverse">
        <a href={bookHref} className="btn-apex flex-1">
          Create account &amp; pick a time
        </a>
      ...
      <p className="mt-3 text-[12px] leading-relaxed text-subtle">
        Takes a minute — we already have your vehicle and quote, and you&rsquo;ll only see times
