...
{
  // Interior Detail offers no add-ons; the clay bar must be dropped, not priced.
  const { data } = await post({
    ...validAutomotive,
    serviceIds: ['auto-interior-detail'],
    addOnIds: ['auto-clay-bar'],
  });
  check(
    'unattachable add-on is dropped (interior detail, 3-row SUV = $130)',
    data.quotedTotal === 130,
    `got ${data.quotedTotal}`
  );
}
